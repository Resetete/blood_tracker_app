...
[
  ...
  { parameter_name: 'hemoglobin', abbreviations: %w[HB HGB], chart_unit: 'g/dL', upper_limit: 17.5, lower_limit: 12.0 },
  { parameter_name: 'lymphocytes', abbreviations: ['Lymph'], chart_unit: '%', upper_limit: 40.0, lower_limit: 20.0 },
  { parameter_name: 'lymphocytes_absolute_count', abbreviations: ['ALC'], chart_unit: '10^3/μL', upper_limit: 3.6, lower_limit: 1.0 },
  ...
  { parameter_name: 'thrombozythes', abbreviations: %w[PLT Thrombos], chart_unit: '10^3/μL', upper_limit: 450.0,
    lower_limit: 150.0 },
  { parameter_name: 'white_blood_cells', abbreviations: %w[WBC Leukocytes], chart_unit: '10^3/μL', upper_limit: 11.0,
    lower_limit: 4.0 }
].each do |parameter_attr|
  parameter = Admin::Hemigrams::ParameterMetadata.find_or_initialize_by(parameter_name: parameter_attr[:parameter_name])

  if parameter.new_record?
    Rails.logger.debug("Creating ParameterMetadata #{parameter_attr[:parameter_name]}")
  else
    Rails.logger.debug("Updating ParameterMetadata #{parameter_attr[:parameter_name]}")
  end
